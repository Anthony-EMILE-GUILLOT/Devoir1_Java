...
public class DevoirController implements Initializable {
    ...
    public void lvCategoriesClicked(Event event)
    {
        ...
        {
            if(lvCategories.getSelectionModel().isSelected(0))
            {
                tvPlats.getItems().clear();
                tvPlats.getItems().addAll(lesPlats.get("Entrées"));
            }
            if(lvCategories.getSelectionModel().isSelected(1))
            {
                tvPlats.getItems().clear();
                tvPlats.getItems().addAll(lesPlats.get("Plats"));
            }
            if(lvCategories.getSelectionModel().isSelected(2))
            {
                tvPlats.getItems().clear();
    ...
    public void btnAjouterClicked(Event event)
    {
        ...
        {
            alert.setContentText("Sélectionner un plat");
            alert.showAndWait();
        }
        else if (tvCartes.getSelectionModel().getSelectedItems() == null)
        {
            alert.setContentText("Sélectionner une carte");
            alert.showAndWait();
        }
        else if (lvMenus.getSelectionModel().getSelectedItem() == null)
        {
            alert.setContentText("Sélectionner un menu");
            alert.showAndWait();
        }
    }
